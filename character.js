...
const renderCharacter = character => {
  ...
  homeworldSpan.innerHTML = `<a href="/planet?id=${character?.homeworld.id}\">${character?.homeworld.name}</a>`;
  const filmsLis = character?.films?.map(film => `<li><a href="/film?id=${film.id}">${film.title}</li>`)
